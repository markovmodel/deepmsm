...
class NetworkPlot(object):
    ...
    def _draw_arrow(
        ...
        patchA=None, patchB=None, shrinkA=0, shrinkB=0, arrow_label_size=None, add_term=0.01):
        """
        ...
        ptext = center + offset * vnorm + add_term
        self.ax.text(
        ...
        **textkwargs):
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                if (i+j) == 3:
                    if i==0:
                        add_term = _np.array([-0.05,0.])
                    if j==3:
                        add_term = _np.array([0.02,0.])
                    if i==2:
                        add_term = _np.array([0.0,-0.2])
                if (abs(self.A[i, j]) > 0):
                    if _np.abs(state_sizes[i] * self.A[i,j]) > arrow_threshold:
                        ...
                        self._draw_arrow(
                            ...
                            label=arrow_label_format%self.A[i, j], width=arrow_scale * self.A[i, j],
                            arrow_curvature=arrow_curvature, patchA=circles[i], patchB=circles[j]  ,
                            shrinkA=3, shrinkB=0, arrow_label_size=arrow_label_size, color=color, add_term=add_term)
                if (abs(self.A[j, i]) > 0):
                    if _np.abs(state_sizes[i] * self.A[j,i]) > arrow_threshold:
                        if (self.A[j,i] < 0.) or (self.A[j,i]>1.):
                            color ='red'
                        else:
                            color ='grey'
                        self._draw_arrow(
                            self.pos[j, 0], self.pos[j, 1], self.pos[i, 0], self.pos[i, 1], Dx, Dy,
                            label=arrow_label_format%self.A[j, i], width=arrow_scale * self.A[j, i],
